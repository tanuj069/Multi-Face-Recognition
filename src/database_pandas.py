...
4. Cam Name
5. Cam IP
6. Face Distance - the distance between the face and the known face (which can be considered as inverse of confidence)
...
def store_inferred_face_in_dataframe(name_of_person, face_distance, cam_name='unknown', cam_ip='unknown'):
    ...
    if name_of_person not in df_inferred_faces['name/id'].values:
        ...
        df_inferred_faces.loc[len(df_inferred_faces)] = [name_of_person, current_time, current_date, cam_name, cam_ip,
                                                         face_distance]
...
def store_dataframe_in_csv():
    ...
    try:
        ...
        logger.error(f'Exception occurred while storing dataframe in csv file: {e}')
        return False
    finally:
        # reset the dataframe
        # df_inferred_faces = pd.DataFrame(columns=['name/id','time','date','location','face_distance'])
